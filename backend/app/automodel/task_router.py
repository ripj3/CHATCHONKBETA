...
class TaskRouter:
    ...
    def _initialize_fallback_chains(self) -> None:
        ...
        self._fallback_chains = {
            TaskType.TEXT_GENERATION: [
                ...
                ProviderType.DEEPSEEK, ProviderType.QWEN, ProviderType.HUGGINGFACE
            ],
            TaskType.SUMMARIZATION: [
                ProviderType.ANTHROPIC, ProviderType.OPENAI, ProviderType.MISTRAL,
                ProviderType.QWEN, ProviderType.HUGGINGFACE
            ],
            TaskType.TOPIC_EXTRACTION: [
                ProviderType.ANTHROPIC, ProviderType.OPENAI, ProviderType.HUGGINGFACE,
                ProviderType.MISTRAL, ProviderType.QWEN
            ],
            TaskType.CLASSIFICATION: [
                ProviderType.HUGGINGFACE, ProviderType.OPENAI, ProviderType.ANTHROPIC,
                ProviderType.MISTRAL, ProviderType.QWEN
            ],
            TaskType.EMBEDDING: [
                ProviderType.OPENAI, ProviderType.HUGGINGFACE
            ],
            TaskType.SENSEMAKING: [
                ProviderType.ANTHROPIC, ProviderType.OPENAI, ProviderType.DEEPSEEK,
                ProviderType.MISTRAL, ProviderType.QWEN
            ],
            TaskType.PLANNING: [
                ProviderType.ANTHROPIC, ProviderType.OPENAI, ProviderType.DEEPSEEK,
            ...
            TaskType.TRANSLATION: [
                ProviderType.QWEN, ProviderType.OPENAI, ProviderType.ANTHROPIC,
                ProviderType.MISTRAL, ProviderType.HUGGINGFACE
            ],
            TaskType.CHAT: [
                ProviderType.OPENAI, ProviderType.ANTHROPIC, ProviderType.MISTRAL,
                ProviderType.DEEPSEEK, ProviderType.QWEN, ProviderType.HUGGINGFACE
    ...
    async def route_task(
        ...
        **kwargs
        ...
        candidate_models = self._get_candidate_models(
            task_type, priority, preferred_providers, excluded_providers, model_requirements
        )
        ...
        for model in candidate_models:
            try:
                ...
                self._load_balancing[model.provider] = self._load_balancing.get(model.provider, 0) + 1
                
                logger.info(f"Task {task_type} completed successfully using {model.name}")
                return response
        ...
        raise Exception(f"All models failed for task {task_type}. Last error: {str(last_error)}")
    ...
    def _meets_requirements(self, model: Model, requirements: Dict[str, Any]) -> bool:
        ...
        for req_key, req_value in requirements.items():
            if req_key == "min_max_tokens" and model.max_tokens < req_value:
                return False
            elif req_key == "supports_vision" and req_value and not model.supports_vision:
                return False
            elif req_key == "supports_functions" and req_value and not model.supports_functions:
                return False
            elif req_key == "max_cost_per_1k_tokens" and model.cost_per_1k_tokens and model.cost_per_1k_tokens > req_value:
                return False
    ...
    def get_task_capabilities(self, task_type: TaskType) -> Dict[str, Any]:
        ...
        capabilities = {
            ...
            "min_cost_per_1k_tokens": min(
                model.cost_per_1k_tokens for model in models 
                if model.cost_per_1k_tokens is not None
            ) if any(model.cost_per_1k_tokens is not None for model in models) else None
        }
