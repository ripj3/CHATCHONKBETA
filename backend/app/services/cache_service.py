...
class CacheService:
    ...
    def _init_cloudflare(self):
        ...
        try:
            ...
            if cf_api_token and cf_account_id and cf_namespace_id:
                self._cf_api_token = cf_api_token.get_secret_value() if hasattr(cf_api_token, 'get_secret_value') else cf_api_token
            ...
            logger.warning(f"Failed to initialize Cloudflare KV, falling back to in-memory cache: {e}")
            self._use_cloudflare = False
    ...
    async def get(self, key: str) -> Optional[str]:
        ...
        if self._use_cloudflare and self._http_client:
            try:
                response = await self._http_client.get(f"{self._cf_base_url}/values/{key}")
                if response.status_code == 200:
                    return response.text
                elif response.status_code == 404:
                    return None
                else:
                    logger.warning(f"Cloudflare KV get failed with status {response.status_code}")
                    self._use_cloudflare = False
            except Exception as e:
                logger.warning(f"Cloudflare KV get failed, falling back to in-memory: {e}")
                self._use_cloudflare = False
        ...
        if self._use_cloudflare and self._http_client:
            try:
                ...
                if response.status_code in [200, 201]:
                    ...
                    logger.warning(f"Cloudflare KV set failed with status {response.status_code}")
                    self._use_cloudflare = False
            except Exception as e:
                logger.warning(f"Cloudflare KV set failed, falling back to in-memory: {e}")
                self._use_cloudflare = False
